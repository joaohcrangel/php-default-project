...
(function($){
  ...
  $.fn.extend({ 
    ...
    uploadPhoto:function(options) {
      ...
      $("head")
        .append('<link rel="stylesheet" href="/res/theme/material/global/vendor/cropper/cropper.css" />')
        .append('<link rel="stylesheet" href="/res/theme/material/base/assets/examples/css/forms/image-cropping.css" />');
      ...
      return this.each(function() {
        ...
        $el.on("click", function () {
          ...
          var $inputFile = $('<input type="file" id="uploadPhoto-file" accept="image/*">');

          $("#uploadPhoto-file").remove();
          ...
          $inputFile.on("change", function (e) {
            ...
            reader.addEventListener("load", function () {
              ...
              var modal = $.modal({
                id:"modal-upload-photo-crop",
                title:"Ajustar Imagem",
                modalCls:"modal-primary",
                body:
                  '<div class="example-loading example-well h-200 vertical-align text-xs-center">'+
                    '<div class="loader vertical-align-middle loader-circle"></div>'+
                  '</div>'+
                  '<img id="uploadPhoto-photo" style="width: 100%; height:100%; display:none;">',
                buttons:[{
                  text:"Salvar Foto",
                  cls:"btn-primary",
                  click:function(e){
                    ...
                    var data = $("#uploadPhoto-photo").data('cropper').getCropBoxData();

                    var formData = new FormData();
                    formData.append(o.inputName, file);

                    formData.append("width_responsive", $("#uploadPhoto-photo").next('.cropper-container').width());
                    formData.append("height_responsive", $("#uploadPhoto-photo").next('.cropper-container').height());
              ...
              $("#modal-upload-photo-crop .example-loading").hide();

              $("#uploadPhoto-photo").show().prop("src", reader.result).cropper({
                responsive: true,

...
class AppCalendar extends Site {
  ...
  handleFullcalendar() {
    let my_events = [{
      ...
      backgroundColor: getColors("cyan", 600),
      borderColor: getColors("cyan", 600)
    }, {
      id: 999,
      title: 'Repeating Event',
      start: '2016-10-09T16:00:00',
      backgroundColor: getColors("red", 600),
      borderColor: getColors("red", 600)
    }, {
      title: 'Conference',
      start: '2016-10-11',
      end: '2016-10-13'
    ...
    let my_options = {
      ...
      eventClick(event) {
        let color = event.backgroundColor ? event.backgroundColor : getColors('blue', 600);
        ...
        $('#editColor [type=radio]').each(function() {
          let $this = $(this),
            ...
            value = getColors(_value[0], _value[1]);
          if (value === color) {
        ...
        $('#editNewEvent').modal('show').one('hidden.bs.modal', (e) => {
          ...
          color = getColors(color[0], color[1]);
    ...
    let _options;
    let my_options_mobile = Object.assign({}, my_options);
  ...
  handleEventList() {
    ...
    $('.calendar-list .calendar-event').each(function() {
      ...
      $this.data('event', {
        ...
        backgroundColor: getColors(color[0], color[1]),
        borderColor: getColors(color[0], color[1])
      });
      $this.draggable({
        zIndex: 999,
        revert: true,
        ...
        appendTo: '.page',
        helper() {
          return '<a class="fc-day-grid-event fc-event fc-start fc-end" style="background-color:' + getColors(color[0], color[1]) + ';border-color:' + getColors(color[0], color[1]) + '">' +
